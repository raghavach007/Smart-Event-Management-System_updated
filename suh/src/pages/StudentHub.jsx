...
import { 
  Search, X, Share2, Terminal, Check, Trash2, Megaphone 
} from 'lucide-react';
...
export default function StudentHub() {
  ...
  const fetchHubData = useCallback(async () => {
    try {
      ...
      if (token) {
        try {
          const [ticketsRes, requestsRes, announcementsRes] = await Promise.all([
            axios.get('http://localhost:5000/api/student/tickets', authConfig),
            axios.get('http://localhost:5000/api/student/team-requests', authConfig),
            axios.get('http://localhost:5000/api/student/announcements/all', authConfig)
          ]);
  ...
  const handleRegister = async (event) => {
    try {
      ...
      await axios.post(`http://localhost:5000/api/student/events/${event.id}/register`, { teamName }, authConfig);
      await fetchHubData(); 
  ...
  const respondToInvite = async (requestId, status) => {
    try {
      await axios.put(`http://localhost:5000/api/student/team-requests/${requestId}`, { status }, authConfig);
      await fetchHubData();
  ...
  return (
    <div className={styles.masterContainer}>
      ...
      <Navbar 
        ...
        hasNotifications={teamRequests.length > 0 || announcements.length > 0}
      />
        ...
        <>
          ...
          <div className={styles.drawer}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
              ...
              <X onClick={() => setShowNotifications(false)} style={{ cursor: 'pointer' }} />
            </div>

            <div style={{ marginBottom: '2rem' }}>
              <h4 style={{ fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--accent-color)', marginBottom: '1rem' }}>Recruitment Pings</h4>
              {teamRequests.length === 0 ? <p style={{ fontSize: '0.9rem', opacity: 0.6 }}>No active recruitment pings.</p> : teamRequests.map(req => (
                <div key={req.id} className={styles.notificationCard}>
                  <p><strong>{req.sender_name}</strong> invited you to join <strong>{req.team_name}</strong>.</p>
            ...
            <div>
              <h4 style={{ fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--accent-color)', marginBottom: '1rem' }}>Live Updates</h4>
              <AnnouncementsList />
            </div>
          </div>
        </>
